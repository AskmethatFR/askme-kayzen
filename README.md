...
ticket, not restated here. One is worth calling out: **NC1** removes `--rustc-args`
and forces a clean rebuild, and the bundle then fails the alignment check — the
control that proves the flag is load-bearing, not decorative.

Same one-time toolchain setup as the local debug path above, plus `NDK_HOME` pointing
at the same NDK — the script reads it directly rather than deriving it from
`ANDROID_HOME`, so it works unchanged against the different NDK version CI installs
(see `.github/workflows/ci.yml`'s `android-aab` job).
