...
The detail screen's recap (habit-stats, slice 8) was removed on the owner's ruling
(2026-09-21): it told a whole-life story — days done, other days, minutes accumulated, growths
and lightenings — on the screen whose job is to *act* on one habit, and every figure it carried
is already told better by the Week screen (F-9). Its scenarios, tests and catalogue keys went
with it; no other feature replaced it.
...
- **Android runs, but only by hand** — the app builds, installs over USB and keeps habits in the device's own app-private directory, so F-10 holds there like anywhere else. What is missing is the way to *give* it to anyone: no store listing, no signing key, no account (issue #28). An install today is a developer plugging in a cable.
...
- Anchoring is now two-way: the Ancrées screen offers « La remettre dans mon quotidien » on each anchored habit (`[[readmit-habit]]`, slice 7), refusable on a full daily life (« Le quotidien est complet · pour la remettre, ancrez-en une autre d'abord ») or on a title already retaken (« Elle est déjà dans votre quotidien »). The screen's parallel-count footer « Vous suivez N / 5 habitudes en parallèle » is shipped; its per-habit dots remain deferred (see F-7). The detail screen's recap (habit-stats) was **retired** on 2026-09-21 — see F-8.
