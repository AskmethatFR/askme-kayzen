...
| **Step history (*croissance en escalier*)** | The dated staircase of a habit's goal changes, seeded at creation. **Append-only**: both gestures add a step, none is ever removed, merged, or undone — two changes on the same day stay two steps. The current goal is always the last one, never stored apart. | `core/src/habit_management/domain/step_history.rs` |
