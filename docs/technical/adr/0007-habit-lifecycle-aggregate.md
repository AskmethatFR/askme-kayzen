...
| Step history & dose | Dated `StepHistory` VO = ordered `Vec<StepChange { on: LocalDate, dose: Dose }>`, **seeded** at creation with `(creation_date, initial_duration)`. `current_dose() = steps.last().dose` — **never stored separately**. `grow()` pushes a step `+1`; `lighten()` pushes `max(1, current-1)`. `Dose` is a VO **distinct** from `InitialDuration` (different rules: `InitialDuration` keeps the ≤5-min creation guard; `Dose` is a running value, floor 1, no ceiling) | planned: `domain/step_history.rs`, `domain/dose.rs` |
| Floor-of-1 invariant | The floor at 1 min is a **true aggregate invariant**, enforced **twice**: by `Dose` construction (cannot build a `Dose < 1`) AND by `lighten()` computing `max(1, current-1)`. Realizes the floor that [[adr-0005-progression-suggestion-policy]] assigned to the aggregate | — |
...
| Read-side compatibility | Slice-1 reads stay stable: `id`/`title` unchanged; minutes read via a `current_dose()` accessor **now** (returns `initial_duration` until step history lands → zero rework in slice 3). `done_today` source changes from the `false` default to `completion_history.contains(clock.today())` in slice 2 — the Today query must accept an **injected `Clock`** in slice 2 | — |
...
| Reusing `InitialDuration` for the running dose | Different rules — `InitialDuration` has the ≤5-min creation guard; the running `Dose` is floor-1 / no-ceiling. Conflating them would smuggle the creation guard into a running value |
...
- **MUST**: derive the current dose from `StepHistory.last()` via `current_dose()` — never store the current dose separately.
- **MUST**: enforce the floor at 1 both in `Dose` construction and in `lighten()` (`max(1, current-1)`).
...
- **MUST NOT**: create a second aggregate for the lifecycle, or reuse `InitialDuration` as the running dose.
