...
| One exclusion, named | Only `app/src/main.rs` is excluded — it launches Dioxus and holds no behavior a test could pin. Everything else in both crates is mutated, including the views: they carry tests | `.cargo/mutants.toml` |
...
| Excluding the whole `app` crate | Written first, on a miscount. The app crate has 9 tests over its routing, its Today and Add views and its wiring service; 6 of its 12 mutants land on that tested code |
