...
- **`LifecycleState` enum — complete since slice 6: `Active`, `Paused`, `Anchored`** (illegal combos unrepresentable; no `Default` derive — `Habit::new` names `Active` explicitly). One variant landed per slice, each with the use case that transitions into it, and the bet paid: slice 6's new variant broke every exhaustive `match` and handed the developer the exhaustive worklist. `Habit::pause()` / `resume()` / `anchor()` are the three transitions, driven by `PauseHabit` / `ResumeHabit` / `AnchorHabit`; **each one guards itself** since PR 2 — `pause()` requires `Active`, `resume()` requires `Paused`, `anchor()` requires `Active`, all three returning `Result<(), TransitionError>` ([[adr-0007-habit-lifecycle-aggregate]] AD-9, amending AD-4). **What is legal is the aggregate's business; what is offered is still the screen's.** The 9-cell table has 3 legal cells and 6 refusals; `Paused → Anchored` is refused by human ruling (*« une habitude au repos n'est pas pratiquée, donc on la reprend d'abord »*), so the path is `Paused → Active → Anchored`. `TransitionError` never crosses the crate boundary — each use case maps it to a flat variant of its own error (`PauseHabitError::NotActive`, `ResumeHabitError::NotPaused`, `AnchorHabitError::NotActive`). `toggle_done` never inspects the state (paused *and* anchored habits stay markable-done; `MarkDone` is byte-for-byte unchanged since slice 5).
...
- **No lifecycle transition can break the 5-seat cap — and that is now provable.** Let **N** be the habits whose `state() != Anchored` (the cap's predicate, and the only counting site in the workspace: `core/src/habit_management/use_cases/add_habit.rs`, line 72). Production writes `state` in exactly **four** places (`core/src/habit_management/domain/habit.rs`, lines 60/88/96/104): `Habit::new` is **+1** and is the one `AddHabit` checks before saving; `pause()` and `resume()` are **0**; `anchor()` is **−1**. Since PR 2 every one of them is guarded, so **no transition increases \|N\|** and `|N| ≤ 5` holds by induction. This closes the LOW, pre-existing finding Security filed against the `drop-habit-board` refactor — an unguarded `resume()` accepting `Anchored → Active` was the *only* transition that could grow the count, unreachable through the UI at the time and **reachable exactly at slice 7**, which is why the guard landed first ([[adr-0007-habit-lifecycle-aggregate]] AD-9). Transition-table legality **is** a genuine `Habit` invariant (one instance, no other aggregate consulted) — the counter-example [[adr-0013-set-based-validation-outside-aggregates]] closes on, now landed.
- **One honest UX delta from the guards, recorded not fixed.** `app/src/views/habit_detail.rs`'s `Active` branch renders « Mettre en pause » and « L'ancrer » simultaneously (lines 69-86). Clicked fast enough that both dispatch against a stale VDOM, the semantics move from **last wins** to **first wins** (pre-PR `pause→anchor` ended `Anchored`; it now ends `Paused`, the anchor refused). The cap is unaffected in either order — both outcomes are stricter or neutral. Whether Dioxus actually delivers both in one frame is unverifiable here: there is **no click-dispatch harness** in this repo ([[adr-0009-quality-gates]] L1-bis).
- **`HabitRepository`** gains `get(&HabitId) -> Option<Habit>` + upsert-by-id `save` (slice 2).

Still planned: **readmission** (slice 7). It is now a **lifecycle transition on one aggregate**, the mirror of anchoring — not a re-admission onto anything. Its rejection path is real and is **set-based validation**: a readmitted habit re-enters the daily life, so it must pass the same two guards `AddHabit` runs (the title may have been retaken while it was anchored — `readmit-habit` S3 — and the daily life may be full). PR 2's transition-table guard **landed first** (2026-08-17), because slice 7 is the cycle that makes the resume-an-anchored-habit path reachable ([[adr-0013-set-based-validation-outside-aggregates]], and the 2026-08-17 amendment of [[adr-0012-synchronous-cross-aggregate-coordination]]). Two constraints slice 7 inherits, both in [[adr-0007-habit-lifecycle-aggregate]] AD-9: `ReadmitHabit` is the **first transition since `AddHabit` that increases the seat count**, so it falls outside PR 2's induction and must re-apply the set check itself (order: load → set check → `readmit()` → **one** save); and **`Habit::resume()`'s guard must stay exactly `!= Paused`** — widening it so `ResumeHabit` can serve readmission re-opens the closed finding verbatim.
